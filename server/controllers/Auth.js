...
exports.changePassword = async (req, res) => {
  try {
    // fetch data from req body
    const { email, oldPassword, changePassword } = req.body;
    // get oldPass , NewPass , ConfirmPass
    if (!email || !oldPassword || !changePassword) {
      return res.status(403).json({
        success: false,
        message: "All fields are required to fill ,Please try Again",
      });
    }
    // find user in db to match password
    ...
    if (await bcrypt.compare(oldPassword, user.password)) {
      ...
      await mailSender(
        email,
        "Passsword Changed Succesfully",
        "Password has been changed , If Its Not You Please Complain"
      );
      //return res
      return res.status(200).json({
        success: true,
